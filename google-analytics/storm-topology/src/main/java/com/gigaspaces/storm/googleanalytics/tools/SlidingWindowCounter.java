...
public final class SlidingWindowCounter<T> implements Serializable {
    ...
    public void incrementCount(T obj, long delta) {
        objCounter.incrementCount(obj, headSlot, delta);
    }
    ...
    public long[] getOrderedWindowThenAdvance(T obj) {
        ...
        for (int i = 0; i < window.length; i++){
            window[i] = slots[localHead];
            localHead = slotAfter(localHead);
        }

        ArrayUtils.reverse(window);
